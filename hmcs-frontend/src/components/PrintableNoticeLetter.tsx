...
export const PrintableNoticeLetter = forwardRef<HTMLDivElement, NoticeLetterProps>(({
  ...
  const societyNameSi = tenantInfo?.societyNameSi;
  const addressSi = tenantInfo?.addressSi;
  const telephone = tenantInfo?.telephone;
  ...
  const memberName = member?.fullNameSinhala || member?.fullName || member?.nameWithInitials || '........................................................';
  ...
  const arrearsAmountStr = overdueAmount ? Number(overdueAmount).toLocaleString('en-US', { minimumFractionDigits: 2 }) : (loan?.overdueAmount ? Number(loan.overdueAmount).toLocaleString('en-US', { minimumFractionDigits: 2 }) : '....................');
  const totalDueStr = totalDue ? Number(totalDue).toLocaleString('en-US', { minimumFractionDigits: 2 }) : arrearsAmountStr;
  ...
  return (
    <div ref={ref} className="p-10 bg-white text-black font-serif text-sm leading-relaxed max-w-[800px] mx-auto print:p-8 print:max-w-none">
      ...
      {noticeType === 1 && (
        <div className="space-y-6 text-justify">
          <div className="border-b-2 border-black pb-3 text-center space-y-1">
            <h2 className="text-xl font-bold uppercase">{societyNameSi || 'විවිධ සේවා සමූපාකාර සමිතිය'}</h2>
          ...
          <div className="flex justify-between items-start pt-2">
            <div>
              ...
              <p className="text-xs">{memberAddress}</p>
            </div>
          ...
          <p className="indent-8">
            ඔබ විසින් මෙම සමිතියේ <strong>........................................................</strong> ග්‍රාමීය බැංකුවෙන් <strong>{formattedNoticeDate}</strong> දින ලබාගෙන ඇති රු. <strong>{requestedAmount}</strong> ක ණය මුදලේ නියමිත වාරික මසපතා නොගෙවීම නිසා අද දිනට රු. <strong>{arrearsAmountStr}</strong> ක හිඟ ශේෂයක්ද, මෙයට අමතරව <strong>........................</strong> දින සිට රු. <strong>........................</strong> පොලියද අයවිය යුතුව ඇත. එබැවින් කරුණාකර <strong>{formattedDueDate}</strong> දිනට පෙර මෙම හිඟ මුදල හා පොලියද ග්‍රාමීය බැංකුවේ තැන්පත් කරන ලෙස ඔබට මෙයින් මතක් කරමි.
          </p>
      ...
      {noticeType === 2 && (
        <div className="space-y-6 text-justify">
          ...
          <p className="indent-8">
            දිනදී දරන රු. <strong>{requestedAmount}</strong> දරන ඔබට ප්‍රදානය කරන ලද ණය මුදල වෙනුවෙන් දිනට තවම අප වෙත ලැබිය යුතු රු. <strong>{arrearsAmountStr}</strong> ක හිඟ වාරිකයන් මෙතෙක් ගෙවා නොමැති බව දන්වා සිටිමු.
          </p>
      ...
      {noticeType === 3 && (
        <div className="space-y-6 text-justify">
          ...
          <p className="indent-8 font-bold text-center border-y border-dashed border-gray-400 py-1">
            {memberName} මහතා/මහත්මියගේ රු. {requestedAmount} ක ණය මුදල - ණය අංක: {loanNumber}
          </p>

          <p className="indent-8">
            ඔබ විසින් ඇප වී සිටින <strong>{memberName}</strong> මහතාට/මහත්මියට අනුමත කරන ලද ණය මුදල වෙනුවෙන් දිනට ගෙවිය යුතු වූ රූපියල් <strong>{arrearsAmountStr}</strong> ක වාරිකයන් මෙතෙක් ගෙවා නොමැති බව දන්වා සිටිමු.
          </p>
      ...
      {noticeType === 5 && (
        <div className="space-y-5 text-justify text-red-700 border-4 border-red-600 p-6 rounded-lg bg-red-50/20">
          <div className="border-b-2 border-red-600 pb-3 text-center space-y-1">
            <h2 className="text-xl font-black uppercase text-red-700">{societyNameSi || 'විවිධ සේවා සමූපාකාර සමිතිය'}</h2>
          ...
          <div className="flex justify-between items-start pt-2 text-xs text-red-800">
            <div>
              <p><strong>ණයකරු:</strong> {memberName} - {memberAddress}</p>
            </div>
            <div className="text-right">
              <p>දිනය: <span className="font-bold">{formattedNoticeDate}</span></p>
            </div>
