...
BEGIN
    ...
    FOR rec IN (SELECT member_id, created_at FROM member_service.members WHERE registered_branch_id = 1 ORDER BY random() LIMIT 10) LOOP
        ...
        INSERT INTO loan_service.loans (
            ...
            1, 'DISBURSED', 'ACTIVE', v_created_at::date, v_created_at, 1, v_loan_amount, 'LN100' || floor(random()*9999)::text, 'CASH'
        );
    ...
    FOR rec IN (SELECT member_id, created_at FROM member_service.members WHERE registered_branch_id = 1 ORDER BY random() LIMIT 10) LOOP
        ...
        v_ticket_num := 'PW100' || floor(random() * 9999)::text;
        ...
        INSERT INTO pawning_service.pawn_tickets (
            ...
            v_assessed_value, v_advance_amount, 15.0, 1, '59c75e11-b5f5-4ded-b587-d50f47aaee4a', -- Using senior_seenigama user_id
