...
BEGIN
    ...
    TxnSummaries AS (
        SELECT DISTINCT ON (account_id, transaction_timestamp::date)
            account_id, 
            transaction_timestamp::date as txn_date,
            balance_after as end_of_day_balance
        FROM account_service.transactions
        ORDER BY account_id, transaction_timestamp::date, transaction_timestamp DESC, transaction_id DESC
    ),
    JoinedDays AS (
        SELECT 
            ad.account_id, 
